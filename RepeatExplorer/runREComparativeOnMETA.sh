...
#PBS -l select=1:ncpus=16:mem=512gb:scratch_local=128gb
#PBS -j oe
...
#this uses the R script 'REphylo.R' that has to be in /storage/$server/home/$LOGNAME/HybSeqSource
...
repex="/storage/${server}/home/${LOGNAME}/RepeatExplorerSIF/repex_tarean_0.3.12.sif"
...
cut -f1,2 listComparative.txt > Clist.txt
cut -f2,3 listComparative.txt > ClistGS.txt
#loop over samples
cat Clist.txt | while read -r sample prefix; do
...
./plot_comparative_clustering_summary.R --cluster_table=re_output_comparative${subsample}/CLUSTER_TABLE.csv --comparative_counts=re_output_comparative${subsample}/COMPARATIVE_ANALYSIS_COUNTS.csv --number_of_colors=20 -g ClistGS.txt --output=re_comparative${subsample}_20colors.pdf

#extract data for phylogenetic tree and copy back home
nrsamples=$(cat Clist.txt | wc -l)
